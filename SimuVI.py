import numpy as np
from data.generate_random_dis import generate_samples_dag_with_probs,sample_from_probs
...
def run_power_experiments(sample_sizes=[400,600,800,1000,1200],
                          n_experiments=1, n_sim=100, n_jobs=50,
                          scenario=True,
                          n_t=100, bvals=2000, phi_type='sin', alpha=0.05):
    all_results = []
    
    
    _, probs = generate_samples_dag_with_probs(
            n_samples=10,  # 生成概率分布时样本数无关
            n_U=3, n_W=3, n_X=5, n_Y=3,
            edge_X_to_Y=scenario, seed=43
        )
    
    logging.info(f"Generated {n_sim} random probability distributions for scenario={scenario}")
        ...
        for exp_id in range(n_experiments):
            ...
            sim_results = Parallel(n_jobs=n_jobs)(
                delayed(run_three_methods)(
                    samples= sample_from_probs(probs, n_samples=n_samples),  # 每次重复内部也可以重新采样
                    n_t=n_t, bvals=bvals, 
            ...
            logging.info(f"n_samples: {n_samples}, exp_id: {exp_id}, power_method1: {power_method1}, power_method2: {power_method2}, power_method3: {power_method3}")
