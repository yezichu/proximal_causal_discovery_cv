...
from discrete.liu.estimator import margEstimate,condEstimate
...
class ProxyTest():
    def __init__(self, pddata, levelx, levelws, levely, eps=1e-5):
        ...
        if levely != self.pddata['Y'].unique().shape[0]:
            self.levely = self.pddata['Y'].unique().shape[0]
        levels = 1
        for wname in self.wnames:
            levels *= self.pddata[wname].unique().shape[0]
        # assert np.prod(levelws) == levels
        self.levelx = self.pddata['X'].unique().shape[0]
        self.levelws = [self.pddata[wname].unique().shape[0] for wname in self.wnames]
        # print(f"levelx: {self.levelx}, levelws: {self.levelws}, levely: {self.levely}")
    ...
    def _est_Q(self,):
        ...
        self.Q_hat = self.Q_hat.reshape(np.prod(self.levelws),self.levelx)
        assert matrix_rank(self.Q_hat) == np.prod(self.levelws), 'Warning, the matrix P(W|X) with rank: {} does not have full row rank: {}. Please use get_linind_bins() in binn.py'.format(matrix_rank(self.Q_hat),np.product(self.levelws))
    ...
    def test(self,):
        ...
        # p-value
        freedom = (self.levelx - np.prod(self.levelws)) * (self.levely - 1)
