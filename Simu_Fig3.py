from data.linear_causal_generator import generate_samples
        ...
        for i in tqdm(range(num_runs), desc=f"{causal_key} runs"):
            ...
            for sample_size in sample_sizes:
                ...
                for _ in range(num_samples_per_run):
                    samples = generate_samples(num_samples=sample_size, gamma_x=gamma_x)
...
if __name__ == "__main__":
    ...
    num_runs=20,
    num_samples_per_run=100,
    methods=['kernel_proxy_test', 'moment_proxy_test'],
    output_path='decs_results_II.json',
    use_parallel=True,
    n_jobs=50
)
